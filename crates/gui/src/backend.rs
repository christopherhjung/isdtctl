...
use api::ble::{self, Discovered};
use api::client::default_poll_cycle;
use api::response::{Electrical, HardwareInfo, InnerResistance, Temperature, WorkState};
use api::tokens::{self, ClientId};
use api::{BatteryKind, BleClient, LinkType, Response, TaskType, WriteChannel};
...
async fn bind(device: &Discovered, client_id: ClientId) -> Result<(), api::ClientError> {
...
fn is_fatal(error: &api::ClientError) -> bool {
    matches!(
        error,
        api::ClientError::Link(api::LinkError::Closed | api::LinkError::Transport(_))
    )
}
...
async fn apply(client: &mut BleClient, command: TaskCommand) -> Result<String, api::ClientError> {
    match command {
        TaskCommand::Start {
            task,
            battery,
