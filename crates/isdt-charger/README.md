...
verified against a CM1620. See `PROTOCOL.md` at the repository root for the
format itself, including which parts the application leaves undefined.
...
use isdt_charger::{Client, tokens};
...
```toml
isdt-charger = { version = "0.1", default-features = false }
```
